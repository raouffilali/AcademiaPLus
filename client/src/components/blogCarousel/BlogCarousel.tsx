          ...
          <div key={blog.id}>
            <Link to={`/blog/${blog.id}`}>
              <div className="bg-gray-50 w-60 rounded-xl border border-gray-200 shadow-sm items-start text-center">
                <img className="mx-auto h-56 rounded-t-xl w-full" src={blog.thumbnail} alt={blog.name} />
                <div className="space-y-2 p-6">
                  <div className="mb-1 text-xl font-medium leading-tight">{blog.name}</div>
                  <p className="text-sm text-neutral-500 dark:text-neutral-400">{blog.category}</p>
                  <div className="text-redPal text-sm">
                    <FontAwesomeIcon icon={faCalendarAlt} className="mr-2 inline-block align-middle" />
                    <span className="inline-block align-middle">{blog.datePosted}</span>
                  </div>
                </div>
              </div>
            </Link>
