  ...
  return (
    <div className=" container">
      <div className="shadow rounded bg-white p-4 text-sm">
        ...
        <div className="mb-4">
          <label className="block font-semibold text-gray-600 mb-1">Course Title</label>
          <input
            type="text"
            value={courseTitle}
        ...
        <div className="mb-4">
          <label className="block text-gray-600 font-semibold mb-1">Course Level</label>
          <select
            value={courseLevel}
            onChange={(e) => setCourseLevel(e.target.value)}
            className="border bg-white p-2 rounded w-full"
          >
            <option className="text-gray-500" value="">Select Level</option>
        ...
        <div className="mb-4">
          <label className="text-gray-600 block font-semibold mb-1">Course Description</label>
          <textarea
          ...
          {sections.map((section) => (
            <div key={section.id} className="section">
              <div className="section-header">
                ...
                <button onClick={() => setCurrentSection(section.id)}>Add Lecture</button>
              </div>
...
const Settings: React.FC<{ onPrevious: () => void }> = ({ onPrevious }) => {
  ...
  const handleNewRequirementChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setNewRequirement(e.target.value);
  };

  const handleNewRequirementKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      <div>
        <div className="flex flex-wrap">
          {requirements.map((requirement, index) => (
            <div key={index} className="bg-gray-100 rounded py-1 px-3 m-1 flex items-center">
              <span>{requirement}</span>
              <button className="ml-2 text-lg text-red-600" onClick={() => handleDeleteRequirement(index)}>
                &times;
              </button>
            </div>
          ))}
